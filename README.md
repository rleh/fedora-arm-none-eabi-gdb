...
```sh
podman build --tag fedora-rpmbuild:44 --file Dockerfile
```

Run podman/docker image with spec file and SOURCES/ directory mounted:
```sh
podman run --rm -v ./SOURCES/:/root/rpmbuild/SOURCES/:z -v ./gnu-keyring.gpg:/root/rpmbuild/SOURCES/gnu-keyring.gpg:ro,z -v ./arm-none-eabi-gdb.spec:/root/rpmbuild/SPECS/arm-none-eabi-gdb.spec:ro,z -it fedora-rpmbuild:44
```
